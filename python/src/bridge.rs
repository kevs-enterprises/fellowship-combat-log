...
//! handed to Python.
//!
//! `pyo3`/`pythonize` can serialize any `Serialize` value straight into a Python object, but the
//! precise shape Python callers see — and the discriminated unions in `python/fellowship_combat_log/
//! __init__.pyi` — has to match what actually gets serialized byte for byte. This module's job is
//! to keep those in sync:
//!
//! - every mirror struct derives plain `#[derive(Serialize)]` with **no** `rename_all`: Python's
//!   idiomatic key casing is snake_case, which is already the real Rust field names, so unlike the
//!   TypeScript binding's `camelCase` mirrors there is nothing to rename;
...
//!   variant name string (`"Hit"`, `"CriticalStrike"`, ...) — exactly a `Literal[...]` union;
//! - enums that mix unit and data-carrying variants (`CastPhaseJson`, `EncounterPhaseJson`,
//!   `EventBodyJson`) are internally tagged (`#[serde(tag = "type")]`) so Python gets a
//!   discriminant field. Newtype variants that wrap a whole mirror struct (e.g.
//!   `EventBodyJson::DamageHeal(DamageHealJson)`) flatten automatically under internal tagging
//!   because the wrapped struct serializes as a map.
...
//!
//! Every mirror type has a `From<&real type>` (or `From<real type>`) conversion below, kept in the
//! same order as the real types in `fellowship_combat_log::event` for easy side-by-side comparison.
...
/// Serialize a mirror value into a Python object via `pythonize`. `PythonizeError` already
/// converts into `PyErr`, so the `?` in callers does the right thing without help here.
pub fn to_python<'py, T: Serialize>(py: Python<'py>, value: &T) -> PyResult<Bound<'py, PyAny>> {
    pythonize(py, value).map_err(Into::into)
}

/// Render a `ParseError` as the message a raised `ValueError` carries. `seq` (the same line
/// number the caller passed to `parse_line`) is folded in since the error itself doesn't carry it.
